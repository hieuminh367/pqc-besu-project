...
The core implementation of the legacy relayer-path gateway PoC and the demo-level implementation of
**Plan B2 native PQC transactions** have been completed.
...
```text
[OK] PQC relayer-path gateway PoC implementation completed.
...
[OK] Plan B2 native Besu transaction validation, block inclusion, and EVM execution are verified.
[OK] Frontend -> backend -> ABI wrapper -> native 0x05 -> Besu -> receipt demo flow is stabilized.
...
This directory is used for relayer and native PQC implementation, demo scripts, evidence, frontend, backend, gateway, smart contracts, and documentation.
...
[OK] Interface replaced by real ML-DSA-65 for final relayer demo.
```
...
```text
results/receipts/relayer-b1-valid-mldsa-receipt.json
```
...
## 25. Legacy Relayer Path Status

The main relayer-path Gateway PoC flow is completed.
...
Legacy relayer claim:

```text
This path demonstrates pre-chain ML-DSA-65 transaction verification through a PQC Gateway in front of a real Besu QBFT private network.
```
...
The project must not claim the following for this legacy relayer path:
...
The project has completed the core blockchain execution, PQC Gateway verification, backend direct transaction building, and frontend dashboard for the relayer path.
